...
void MaterialData::Deserialize(Json &json) {
  ...
  for (int i = 0; i < texIndices.size(); ++i) {
    if (texPathes[i] == "::textureSlot") {
      ...
      texVariables.insert(std::make_pair(texIndices[i], Core.RManager.TextureSlot));
    } else {
      // if this texture has some actual data
      texVariables.insert(
          std::make_pair(texIndices[i], Core.RManager.GetTexture(texPathes[i])));
    }
  }
...
void MaterialData::SetupLights(vector<Light> &lights) {
  ...
  for (auto &light : lights) {
    if (light.Type == Light::DIRECTIONAL_LIGHT) {
      ...
      shader->SetVec3(lightDirName, Core.EManager.EntityFromID(light.GetID())->LocalForward);
